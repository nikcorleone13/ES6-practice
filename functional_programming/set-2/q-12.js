...
// (title, director, year, rating) and returns an array with only the movie titles that were made before 1990 and 
// has a rating above 8.0. (Question Level: tough)
...
const bollywoodMovies = [
  ...
  const getBestOldBollywoodMovies = movObj =>{ 
  const movies = movObj.map((movObj) => (movObj.year < 1990 && movObj.rating > 8)?  movObj.title : '');
  
  const finalResult = movies.filter((movies) => movies != 0);
  return finalResult
  }
